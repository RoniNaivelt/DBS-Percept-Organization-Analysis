...
for hemi = {'Left', 'Right'}
    ...
    if isfield(data.DiagnosticData.LFPTrendLogs, ['HemisphereLocationDef_' cell2mat(hemi)]) &&... % Check if data for this side is not empty
...
Patiant_Initials = [data.PatientInformation.Initial.PatientFirstName(1) data.PatientInformation.Initial.PatientLastName(1)];
Patient_name = ['Patient_' Patiant_Initials];
DBS_data.(Patient_name).Info.PatientInfo.Initials = Patiant_Initials;
