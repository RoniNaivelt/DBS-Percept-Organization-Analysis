...
hemi = hemisphere_names{hemiIdx};
    ...
    %%%%%%%%%%%%%%%% create the new data vectors %%%%%%%%%%%%%%%%
        ...
        % handle older data which might have one of the hemispheres in a
        % different place
        if size(Group_params,1) > 1
            if iscell(Group_params)
                curr_group_params = Group_params{hemiIdx};
            else
                curr_group_params = Group_params(hemiIdx);
            end
        elseif size(Group_params,1) < 2
            Groups_sensing_struct = data.Groups.Initial(Group_indx).ProgramSettings;
            Groups_sensing_fields = fieldnames(Groups_sensing_struct);
            if any(contains(Groups_sensing_fields, hemi))
                field_indx = find(contains(Groups_sensing_fields, hemi));
                field_name = Groups_sensing_fields{field_indx};
                curr_group_params = Groups_sensing_struct.(field_name);
            else
                curr_group_params = Groups_sensing_struct.SensingChannel;
            end
        end

        if isstruct(curr_group_params) && numel(fieldnames(curr_group_params)) == 1
            field_name = fieldnames(curr_group_params);
            curr_group_params = curr_group_params.(field_name{:});
        end

        if isfield(curr_group_params, 'PulseWidthInMicroSecond')
            Pulse_width_vec = repelem({curr_group_params.PulseWidthInMicroSecond}, numRows*numCols, 1);
        else
            Pulse_width_vec = num2cell(nan(length(DateTime), 1));
        end

        if isfield(curr_group_params, 'RateInHertz')
            Stim_Rate_vec = repelem({curr_group_params.RateInHertz}, numRows*numCols, 1);
        else
            Stim_Rate_vec = num2cell(nan(length(DateTime), 1));
        end

        if isfield(curr_group_params, 'SensingSetup') && isfield(curr_group_params.SensingSetup, 'FrequencyInHertz')
            Sensing_Freq_vec = repelem({curr_group_params.SensingSetup.FrequencyInHertz}, numRows*numCols, 1);
        else
            Sensing_Freq_vec = num2cell(nan(length(DateTime), 1));
        end

        if isfield(curr_group_params, 'Channel')
            contact = regexp(curr_group_params.Channel, '\.', 'split');
            Contacts_vec = repelem(contact(end), numRows*numCols, 1);
        else
            Contacts_vec = num2cell(nan(length(DateTime), 1));
        end
        ...
        sensing_impedance = [];
        if isfield(curr_group_params, 'ElectrodeState')
            Sensing_electrodes = curr_group_params.ElectrodeState;
        end
